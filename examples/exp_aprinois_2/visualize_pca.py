...
pcs = []
for i in range(0, 20):
    with open(Path(r"examples\exp_aprinois_2\out\pc_" + f"{i+1}.json"), "r") as file:
        pc = json.load(file)
        pcs.append(pc)
...
plt.legend()
plt.savefig(Path(r"examples\exp_aprinois_2\out\screeplot.png"))
plt.clf()

####################################PC SLICES##########################################

for i in range(0, 20):
    path = Path("examples\exp_aprinois_2\out\zscore_pc_" + f"{i+1}" + ".nii")
    ...
    c0 = axs[0, 0].imshow(ndimage.rotate(img[64, :, :].transpose(), 180), cmap='viridis', vmin=min_img, vmax=max_img)
    axs[0, 0].set_title("Saggital Slice at [64, :, :]")
    fig.colorbar(c0, ax=axs[0, 0], orientation='vertical', shrink=0.63)

    # Plot on the second subplot
    c1 = axs[0, 1].imshow(ndimage.rotate(img[:, 72, :].transpose(), 180), cmap='viridis', vmin=min_img, vmax=max_img)
    ...
    data = [
        ...
        [f"{pc['pc_number']+1:.1f}", f"{pc['sv']:.1f}", f"{pc['vaf']:.2f}", f"{pc['accuracy']:.2f}"],
    ]


    # Plot table data
    axs[1, 1].axis('off')  # Turn off the axis for the table subplot
    table = axs[1, 1].table(cellText=data,
                        cellLoc='center',
                        loc='center')
    ...
    plt.savefig(
        Path(r"examples\exp_aprinois_2\out\zscore_pc_" + f"{i+1}" + ".png")
    )
    plt.close()
